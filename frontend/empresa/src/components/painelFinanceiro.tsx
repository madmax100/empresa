...
const FinancialDashboard = () => {
    ...
    useEffect(() => {
        fetchData();
    }, [date, status, debouncedSearchTerm]);
    ...
    const SummaryCard = ({ title, total, count, onClick, isExpanded }: {
        title: string;
        total: number;
        count: number;
        onClick: () => void;
    ...
    const renderTitulosTable = (titulos: any[], tipo: 'receber' | 'pagar') => {
        ...
        return (
            <Table>
                ...
                <TableBody>
                    {sortData(titulos, sortConfig, tipo).map((titulo) => {
