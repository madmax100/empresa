...
export function PagarList() {
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ['pagarList', params],
    queryFn: async () => (await api.get<TituloPagar[]>('contas_pagar/', { params })).data,
  });
    ...
    const get = (t: TituloPagar) => {
      switch (campo) {
        ...
        case 'status':
          return t.status;
        case 'fornecedor':
          return (t.fornecedor?.nome || '').toLowerCase();
        case 'id':
        default:
          return t.id;
      }
  ...
  const [baixaTitulo, setBaixaTitulo] = useState<TituloPagar | null>(null);
  ...
  const baixarMut = useMutation({
    mutationFn: async (vars: { id: number; data_pagamento: string; valor_pago: string }) => {
      return (
        await api.patch(`contas_pagar/${vars.id}/atualizar_status/`, {
          status: 'P',
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['pagarList'] });
      qc.invalidateQueries({ queryKey: ['pagarDashboard'] });
      setBaixarOpen(false);
      setBaixaTitulo(null);
      setDataPagamento('');
      setValorPago('');
  ...
  // Estornar (Contas a Pagar não tem endpoint de estorno no backend; manteremos apenas baixar)
  ...
  <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1.5} sx={{ mb: 2 }}>
        ...
        <TextField
          label="Buscar"
          placeholder="fornecedor, histórico, forma"
          value={busca}
        ...
        >
          ...
          <MenuItem value={'fornecedor_asc'}>Fornecedor (A-Z)</MenuItem>
          <MenuItem value={'fornecedor_desc'}>Fornecedor (Z-A)</MenuItem>
        ...
        <TableBody>
          {sorted.map((t) => (
            <TableRow key={t.id}>
              ...
              <TableCell>{t.fornecedor?.nome ?? '-'}</TableCell>
