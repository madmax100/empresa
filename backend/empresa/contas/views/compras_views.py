...
class ComprasContaPagarPagasResumoView(APIView):
    """Resumo de contas pagas por período (compras)."""
    ...
    def get(self, request, *args, **kwargs):
        ...
        contas = ContasPagar.objects.select_related('fornecedor').filter(status='P')
        ...
        response = {
            ...
            'totais': {
                ...
                'valor_pago': self._to_float(resumo['valor_pago']),
                'valor_total_pago': self._to_float(resumo['valor_total_pago']),
                'juros': self._to_float(resumo['juros']),
                'tarifas': self._to_float(resumo['tarifas'])
            }
        }
...
class ComprasRequisicaoCadastrarView(APIView):
    """Cadastro de requisição de compra com itens."""

    def post(self, request, *args, **kwargs):
        payload = request.data or {}
        requisicao_data = payload.get('requisicao') or {}
        itens_data = payload.get('itens') or []

        numero_requisicao = requisicao_data.get('numero_requisicao')
        if not numero_requisicao:
            return Response(
                {'error': 'numero_requisicao é obrigatório.'},
                status=status.HTTP_400_BAD_REQUEST
            )
        ...
        with transaction.atomic():
            requisicao = RequisicoesCompra.objects.create(
                ...
                solicitante_id=requisicao_data.get('solicitante_id'),
                fornecedor_sugerido_id=requisicao_data.get('fornecedor_sugerido_id'),
                data_solicitacao=requisicao_data.get('data_solicitacao') or timezone.now(),
                prioridade=requisicao_data.get('prioridade'),
                observacoes=requisicao_data.get('observacoes'),
            )

            total_estimado = Decimal('0.00')
            for item in itens_data:
                quantidade = _decimal(item.get('quantidade'), default='0.0000')
                preco_estimado = _decimal(item.get('preco_estimado'))
                valor_total = quantidade * preco_estimado
            ...
            requisicao.valor_estimado = total_estimado
            requisicao.save(update_fields=['valor_estimado'])

        return Response(
            {'id': requisicao.id, 'numero_requisicao': requisicao.numero_requisicao, 'valor_estimado': float(total_estimado)},
            status=status.HTTP_201_CREATED
        )
...
class ComprasRequisicaoAprovarView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        requisicao_id = (request.data or {}).get('requisicao_id')
        if not requisicao_id:
            return Response(
                {'error': 'requisicao_id é obrigatório.'},
                status=status.HTTP_400_BAD_REQUEST
            )

        try:
            requisicao = RequisicoesCompra.objects.get(id=requisicao_id)
        except RequisicoesCompra.DoesNotExist:
            return Response(
                {'error': 'Requisição não encontrada.'},
                status=status.HTTP_404_NOT_FOUND
            )

        if requisicao.status == 'A':
            return Response(
                {'error': 'Requisição já aprovada.'},
                status=status.HTTP_400_BAD_REQUEST
            )
...
class ComprasCotacaoCadastrarView(APIView):
    """Cadastro de cotação de compra."""

    def post(self, request, *args, **kwargs):
        payload = request.data or {}
        cotacao_data = payload.get('cotacao') or {}
        itens_data = payload.get('itens') or []

        fornecedor_id = cotacao_data.get('fornecedor_id')
        ...
        requisicao_id = cotacao_data.get('requisicao_id')
        requisicao = None
        if requisicao_id:
            try:
                requisicao = RequisicoesCompra.objects.get(id=requisicao_id)
        ...
        if not itens_data and requisicao:
            itens_data = list(requisicao.itens.values('produto_id', 'quantidade', 'preco_estimado'))

        if not itens_data:
            return Response(
                {'error': 'Informe itens para a cotação.'},
                status=status.HTTP_400_BAD_REQUEST
            )
        ...
        with transaction.atomic():
            cotacao = CotacoesCompra.objects.create(
                requisicao=requisicao,
                fornecedor_id=fornecedor_id,
                data_cotacao=cotacao_data.get('data_cotacao') or timezone.now(),
                validade=cotacao_data.get('validade'),
                condicoes_pagamento=cotacao_data.get('condicoes_pagamento'),
                observacoes=cotacao_data.get('observacoes'),
            )

            total = Decimal('0.00')
            for item in itens_data:
                quantidade = _decimal(item.get('quantidade'), default='0.0000')
                valor_unitario = _decimal(item.get('valor_unitario') or item.get('preco_estimado'))
                valor_total = quantidade * valor_unitario
                ItensCotacaoCompra.objects.create(
                    ...
                    valor_unitario=valor_unitario,
                    valor_total=valor_total,
                    prazo_entrega_dias=item.get('prazo_entrega_dias'),
                )
                total += valor_total
        ...
        return Response(
            {'id': cotacao.id, 'valor_total': float(total)},
            status=status.HTTP_201_CREATED
        )
...
class ComprasCotacaoAprovarView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        cotacao_id = (request.data or {}).get('cotacao_id')
        if not cotacao_id:
            return Response(
        ...
        try:
            cotacao = CotacoesCompra.objects.select_related('requisicao').get(id=cotacao_id)
        except CotacoesCompra.DoesNotExist:
            return Response(
        ...
        if cotacao.requisicao and cotacao.requisicao.status != 'C':
            cotacao.requisicao.status = 'C'
...
class ComprasPedidoGerarView(APIView):
    """Geração de pedido de compra a partir de cotação ou requisição."""

    def post(self, request, *args, **kwargs):
        payload = request.data or {}
        pedido_data = payload.get('pedido') or {}
        itens_data = payload.get('itens') or []

        cotacao_id = pedido_data.get('cotacao_id')
        requisicao_id = pedido_data.get('requisicao_id')
        ...
        if cotacao_id:
            try:
                cotacao = CotacoesCompra.objects.get(id=cotacao_id)
            except CotacoesCompra.DoesNotExist:
                return Response(
                    {'error': 'Cotação não encontrada.'},
                    status=status.HTTP_404_NOT_FOUND
                )
            fornecedor_id = fornecedor_id or cotacao.fornecedor_id
        ...
        if not itens_data:
            if cotacao:
                itens_data = list(cotacao.itens.values('produto_id', 'quantidade', 'valor_unitario', 'valor_total'))
            elif requisicao:
                itens_data = list(requisicao.itens.values('produto_id', 'quantidade', 'preco_estimado'))

        if not itens_data:
            return Response(
                {'error': 'Informe itens para o pedido.'},
                status=status.HTTP_400_BAD_REQUEST
            )

        numero_pedido = pedido_data.get('numero_pedido') or timezone.now().strftime('PC-%Y%m%d%H%M%S')
        ...
        with transaction.atomic():
            pedido = PedidosCompra.objects.create(
                ...
                cotacao=cotacao,
                fornecedor_id=fornecedor_id,
                data_emissao=pedido_data.get('data_emissao') or timezone.now(),
                data_prevista=pedido_data.get('data_prevista'),
                condicoes_pagamento=pedido_data.get('condicoes_pagamento'),
            ...
            for item in itens_data:
                quantidade = _decimal(item.get('quantidade'), default='0.0000')
                valor_unitario = _decimal(item.get('valor_unitario') or item.get('preco_estimado'))
                desconto = _decimal(item.get('desconto'))
        ...
        return Response(
            {'id': pedido.id, 'numero_pedido': pedido.numero_pedido, 'valor_total': float(total)},
            status=status.HTTP_201_CREATED
        )
...
class ComprasPedidoAprovarView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        pedido_id = (request.data or {}).get('pedido_id')
        if not pedido_id:
            return Response(
        ...
        try:
            pedido = PedidosCompra.objects.get(id=pedido_id)
        except PedidosCompra.DoesNotExist:
            return Response(
...
class ComprasPedidoEnviarView(APIView):
...
class ComprasPedidoDetalheView(APIView):
    """Detalha um pedido de compra com itens."""

    def get(self, request, pedido_id, *args, **kwargs):
        try:
            pedido = PedidosCompra.objects.select_related('fornecedor').get(id=pedido_id)
        except PedidosCompra.DoesNotExist:
            return Response(
                {'error': 'Pedido não encontrado.'},
                status=status.HTTP_404_NOT_FOUND
        ...
        itens = list(
            pedido.itens.select_related('produto').values(
                'id',
                'produto_id',
                'produto__nome',
                'produto__codigo',
        ...
        return Response(
            {
                'pedido': {
                    ...
                    'fornecedor_nome': pedido.fornecedor.nome if pedido.fornecedor_id else None,
...
class ComprasPedidoListaView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        pedidos = PedidosCompra.objects.select_related('fornecedor')

        if status_filtro:
            pedidos = pedidos.filter(status=status_filtro)
        if fornecedor_id:
            pedidos = pedidos.filter(fornecedor_id=fornecedor_id)
        if data_inicio and data_fim:
